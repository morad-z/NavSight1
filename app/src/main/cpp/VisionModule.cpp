...
VisionOutput VisionModule::processFrame(
    ...
    // Need at least 8 points for Essential Matrix estimation (5-point is minimum, 8-point is better)
    ...
    E = cv::findEssentialMat(good_next_corners, good_prev_corners, camera_matrix_,
                             cv::RANSAC, 0.9999, 0.5, inlier_mask);  // Higher confidence, lower threshold
...
cv::Mat VisionModule::integrateGyro(long start_ns, long end_ns) {
    ...
    // Clear old gyro data
    gyro_buffer_.erase(
        std::remove_if(gyro_buffer_.begin(), gyro_buffer_.end(),
                      [end_ns](const GyroData& g) { return g.timestamp_ns <= end_ns; }),
        gyro_buffer_.end()
    );
...
void VisionModule::estimateScaleFromAccel(const cv::Mat& translation, long dt_ns) {
    ...
    // Get average acceleration during this frame (acquires its own lock)
    ...
    // Remove gravity component to get motion acceleration
    float accel_magnitude = std::sqrt(avg_accel.x * avg_accel.x +
                                     avg_accel.y * avg_accel.y +
                                     avg_accel.z * avg_accel.z);
...
bool VisionModule::checkMotionDegeneracy(
    ...
    if (avg_motion < 1.0f) {
        __android_log_print(ANDROID_LOG_WARN, TAG,
            "Motion too small: %.2f pixels", avg_motion);
    ...
    if (motion_variance < 0.1f) {
        __android_log_print(ANDROID_LOG_WARN, TAG,
            "Motion too uniform: variance = %.2f", motion_variance);
