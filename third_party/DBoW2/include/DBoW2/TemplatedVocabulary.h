...
public:
  ...
  /**
   * Loads the vocabulary from a plain-text ORB-SLAM2-style file.
   * NavSight extension: upstream DBoW2 only ships YAML/cv::FileStorage I/O
   * (load()/save()), but the publicly-distributed ORBvoc.txt from ORB-SLAM2
   * is in a custom whitespace-separated format that is much smaller and
   * faster to parse than the equivalent YAML on Android.
   *
   * Format (one line per node, BFS order; the first line is the header):
   *   line 0:        k L scoring_type weighting_type
   *   line 1..N:     parent_id is_leaf b0 b1 ... b31 weight
   *
   *   - parent_id  : int, parent node id; root has -1 (or 0; see below)
   *   - is_leaf    : 0 or 1 — when 1 the node is also a word
   *   - b0..b31    : 32 bytes (each in [0,255]) of the node's mean ORB
   *                  descriptor, written as decimal integers
   *   - weight     : double, IDF weight of the node (used only for words)
   *
   * The line index defines the node id: the header is line 0 and node 0
   * (the root) is implicit; the first node line is node id 1, etc. This
   * matches ORB-SLAM2's writer.
   *
   * Returns true on success, false if the file could not be opened or the
   * header could not be parsed. On failure the vocabulary is left empty.
   *
   * @param filename path to ORBvoc.txt (already extracted from .tar.gz on
   *                 the Android side by Kotlin code).
   */
...
// NavSight extension: ORB-SLAM2-compatible plain-text loader.
// Format:
//   <k> <L> <scoring_type> <weighting_type>
//   <parent_id> <is_leaf> <b0> <b1> ... <b31> <weight>
//   ...
// Node id is the implicit 1-based line index after the header (root is 0).
// Specialised below for FORB only — generic templated path is left
// undefined to avoid pulling parsing semantics into descriptor types we
// don't use (Brief, Surf64).
template<class TDescriptor, class F>
  ...
  const std::string &filename)
{
...

  {
    ...
    int scoring_int = 0;
    int weighting_int = 0;
    if(!(ss >> m_k >> m_L >> scoring_int >> weighting_int)) {
      return false;
    }
    m_scoring   = (ScoringType)scoring_int;
    m_weighting = (WeightingType)weighting_int;
  }

  createScoringObject();

  // Pre-size: expected total number of nodes is bounded by k^L + 1 (root).
  // We don't know the exact count up front, so just push as we read.
  m_nodes.push_back(Node()); // root, id == 0
  m_nodes[0].id = 0;

  // Pre-reserve for the canonical ORB-SLAM2 vocabulary (~145k words plus
  // inner nodes ≈ 170k total). std::vector amortised growth handles
  // vocabularies that overshoot. We deliberately do NOT use k^L:
  // ORB-SLAM2's tree is sparse (~10% fill), and k^L for unusual headers
  // could be pathological (k=10,L=10 → 10G nodes — would blow Android
  // RAM at parse time before we even read any data).
  m_nodes.reserve(170000);
  ...
  while(std::getline(ifs, line))
  {
    ...
    int parent_id = 0;
    int is_leaf   = 0;
    if(!(ss >> parent_id >> is_leaf)) return false;

    // Read 32 descriptor bytes.
    cv::Mat desc(1, F::L, CV_8U);
    for(int j = 0; j < F::L; ++j) {
      int byte_val = 0;
      if(!(ss >> byte_val)) return false;
      desc.at<unsigned char>(0, j) = (unsigned char)byte_val;
    }

    double weight = 0.0;
    if(!(ss >> weight)) return false;

    Node node;
    node.id        = (NodeId)m_nodes.size(); // line index = node id
    node.parent    = (NodeId)parent_id;
    node.weight    = (WordValue)weight;
    ...
    if(is_leaf) {
      // Leaf == word; word ids are assigned in load order.
      const WordId wid = (WordId)m_words.size();
      m_nodes.back().word_id = wid;
      m_words.push_back(&m_nodes.back());
    }
  }

  // m_words may have stale pointers if m_nodes was reallocated mid-load.
  // Fix up by re-walking nodes in order.
  m_words.clear();
  for(size_t i = 1; i < m_nodes.size(); ++i) {
    if(m_nodes[i].isLeaf()) {
      m_nodes[i].word_id = (WordId)m_words.size();
